<!DOCTYPE html>
<html>
<head>
	<g:javascript src="jquery.js"/>
	<asset:javascript src="bootstrap.js"/>
	<asset:stylesheet src="bootstrap.css"/>
   <asset:javascript src="funcionCalendar.js"/>
	<asset:stylesheet src="estiloCalendar.css"/>
	<title>CALENDARIO</title>
</head>
<body>
      <header>
      <div class="container">
            <div class="left">
            <h2>DAMON CF</h2>
            </div>

            <div class="right">
               <div class="right">
                  <div class="dropdown">
                     <button class="btn btn-primary dropdown-toggle btn-group btn-group-lg" type="button" id="dropdownMenu1" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true"> <span class="glyphicon glyphicon-user" aria-hidden="true"></span>Usuario: <span class="caret"></span>
                     </button>
                     <ul class="dropdown-menu" aria-labelledby="dropdownMenu1">
                            <li><a href="#">Perfil</a></li>
                            <li><a href="#">Configuracion</a></li>
                            <li><a href="#">Logout</a></li>
                     </ul>
                  </div>
               </div>                     
            </div>

            <div class="center text-center">
             <H1>Sistema de Inscripción</H1>
            </div>
      </div>
   </header>

   <div class="row">
            <div class="col-md-9">
               <div class="panel panel-primary" id="panel1">
                     <div class="panel-heading">Calendario para anotarse</div>
                        <div sclass="panel-body">
                              <table class="table" id="tbUsuario">
                                    <tr>
                                       <td><span>0</span>/20</td>
                                       <td>Columna 2</td>
                                       <td>Columna 3</td>
                                       <td>Columna 4</td>
                                       <td>Columna 5</td>
                                       <td>Columna 6</td>
                                       <td>Columna 7</td>
                                    </tr>
                                    <tr>
                                       <td>Columna 1</td>
                                       <td>Columna 2</td>
                                       <td>Columna 3</td>
                                       <td>Columna 4</td>
                                       <td>Columna 5</td>
                                       <td>Columna 6</td>
                                       <td>Columna 7</td>
                                    </tr>
                           			<tr>
                                       <td>Columna 1</td>
                                       <td>Columna 2</td>
                                       <td>Columna 3</td>
                                       <td>Columna 4</td>
                                       <td>Columna 5</td>
                                       <td>Columna 6</td>
                                       <td>Columna 7</td>
                                    </tr>
                           			<tr>
                                       <td>Columna 1</td>
                                       <td>Columna 2</td>
                                       <td>Columna 3</td>
                                       <td>Columna 4</td>
                                       <td>Columna 5</td>
                                       <td>Columna 6</td>
                                       <td>Columna 7</td>
                                    </tr>
                           			<tr>
                                       <td>Columna 1</td>
                                       <td>Columna 2</td>
                                       <td>Columna 3</td>
                                       <td>Columna 4</td>
                                       <td>Columna 5</td>
                                       <td>Columna 6</td>
                                       <td>Columna 7</td>
                                    </tr>
                                    <tr>
                                       <td>Columna 1</td>
                                       <td>Columna 2</td>
                                       <td>Columna 3</td>
                                       <td>Columna 4</td>
                                       <td>Columna 5</td>
                                       <td>Columna 6</td>
                                       <td>Columna 7</td>
                                    </tr>
                                    <tr>
                                       <td>Columna 1</td>
                                       <td>Columna 2</td>
                                       <td>Columna 3</td>
                                       <td>Columna 4</td>
                                       <td>Columna 5</td>
                                       <td>Columna 6</td>
                                       <td>Columna 7</td>
                                    </tr>
                                    <tr>
                                       <td>Columna 1</td>
                                       <td>Columna 2</td>
                                       <td>Columna 3</td>
                                       <td>Columna 4</td>
                                       <td>Columna 5</td>
                                       <td>Columna 6</td>
                                       <td>Columna 7</td>
                                    </tr>
                              </table>                                
                        </div>
                     </div>
               </div>
      <div class="col-md-3">
         <div class="panel panel-info">
                     <div class="panel-heading">Clases Anotadas</div>
                        <div class="panel-body">      
                                    <nav>
                                          <ul>
                                             <li>Probando</li>
                                             <li>La lista</li>
                                             <li>Que habia</li>
                                             <li>Creado</li>
                                             <li>A</li>
                                          </ul>
                                    </nav>
                              </div>
                        </div>       
         </div>   
	</div>	
</body>
</html>